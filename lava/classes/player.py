...
class LavaPlayer(DefaultPlayer):
    ...
    async def fetch_and_update_lyrics(self) -> Union[Lyrics[LyricLine], None]:
        ...
        try:
            lrc = await asyncedlyrics.search(
    ...
    async def check_autoplay(self) -> bool:
        ...
        recommendations = await get_recommended_tracks(
            self, self.current, 30 - len(self.queue)
        )
    ...
    async def update_display(
        ...
        self.bot.logger.info(
            "Updating display for player in guild %s in a %s seconds delay",
        ...
        if not self.message and not new_message:
            self.bot.logger.warning(
                "No message to update display for player in guild %s",
                self.bot.get_guild(self.guild_id),
            )
            return
        ...
        if new_message:
            try:
                self.bot.logger.debug(
                    "Deleting old existing display message for player in guild %s",
                    self.bot.get_guild(self.guild_id),
                )
                ...
                ActionRow(
                    (
                        Button(
                            ...
                            label=self.bot.get_text(
                                "display.control.pause", self.locale, "暫停"
                            ),
                        )
                        if not self.paused
                        else Button(
                            ...
                            label=self.bot.get_text(
                                "display.control.resume", self.locale, "繼續"
                            ),
                        )
                    ),
                    Button(
                        ...
                        label=self.bot.get_text(
                            "display.control.previous", self.locale, "重新開始"
                        ),
                    ),
                    Button(
                        style=ButtonStyle.blurple,
                        ...
                        label=self.bot.get_text(
                            "display.control.next", self.locale, "跳過"
                        ),
                    ),
                ),
                ActionRow(
                    Button(
                        ...
                        label=self.bot.get_text(
                            "display.control.stop", self.locale, "停止"
                        ),
                    ),
                    Button(
                        style=ButtonStyle.blurple,
                        ...
                        label=self.bot.get_text(
                            "display.control.rewind", self.locale, "倒帶十秒"
                        ),
                    ),
                    Button(
                        style=ButtonStyle.blurple,
                        ...
                        label=self.bot.get_text(
                            "display.control.forward", self.locale, "快進十秒"
                        ),
                    ),
                ),
                ActionRow(
                    Button(
                        ...
                        label=self.bot.get_text(
                            "display.control.shuffle", self.locale, "隨機播放"
                        ),
                    ),
                    Button(
                        style=[
                        ...
                        label=self.bot.get_text(
                            "display.control.repeat", self.locale, "重複播放"
                        ),
                    ),
                ),
                ActionRow(
                    Button(
                        ...
                        label=self.bot.get_text(
                            "display.control.autoplay", self.locale, "自動播放"
                        ),
                    ),
                    Button(
                        style=(
                            ButtonStyle.green if self.show_lyrics else ButtonStyle.grey
                        ),
                        emoji=self.bot.get_icon("control.lyrics", "🎤"),
                        custom_id="control.lyrics",
                        label=self.bot.get_text(
                            "display.control.lyrics", self.locale, "歌詞顯示"
                        ),
                    ),
                ),
                ActionRow(
                    StringSelect(
                        max_values=4,
                        custom_id="control.filters",
                        placeholder="🎶"
                        + self.bot.get_text(
                            "display.control.filters", self.locale, "套用效果器"
                        ),
                        options=[
                            SelectOption(
                                label=self.bot.get_text(
                                    "filters.rotation", self.locale, "8D 環繞效果"
                                ),
                                value="rotation",
                            ),
                            SelectOption(
                                label=self.bot.get_text(
                                    "filters.tremolo", self.locale, "顫音效果"
                                ),
                                value="tremolo",
                                description=self.bot.get_text(
                                    "command.tremolo.description", self.locale
                                ),
                            ),
                            SelectOption(
                                label=self.bot.get_text(
                                    "filters.vibrato", self.locale, "震音效果"
                                ),
                                value="vibrato",
                                description=self.bot.get_text(
                                    "command.vibrato.description", self.locale
                                ),
                            ),
                            SelectOption(
                                label=self.bot.get_text(
        ...
        if interaction:
            ...
            await self.message.edit(content="", embeds=embeds, components=components)

        self.bot.logger.debug(
            "Updating player in guild %s display message to %s",
            self.bot.get_guild(self.guild_id),
            self.message.id,
        )
    ...
    async def __generate_lyrics_embed(self) -> Embed:
        ...
        if self._lyrics is None:
            return Embed(
                ...
                description=self.bot.get_text(
                    "display.lyrics.loading", self.locale, "正在載入歌詞..."
                ),
        ...
        lyrics_text = (
            "\n".join([f"## {lyric.text}" for lyric in lyrics_in_range]) or "## ..."
        )
    ...
    async def __generate_display_embed(self) -> Embed:
        ...
        if self.is_playing:
            ...
            embed.set_author(
                name=self.bot.get_text(
                    "display.status.disconnected", self.locale, "已斷線"
                ),
        ...
        if self.current:
            ...
            embed.description = (
                f"`{self.__format_time(self.position)}`"
                f" {self.__generate_progress_bar(self.current.duration, self.position)} "
                f"`{self.__format_time(self.current.duration)}`"
            )

            embed.add_field(
                name=self.bot.get_text("display.author", self.locale, "👤 作者"),
                value=self.current.author,
                inline=True,
            )
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    if not self.current.requester
                    else f"<@{self.current.requester}>"
                ),
                inline=True,
            ...
            embed.add_field(
                name=self.bot.get_text(
                    "display.repeat_mode", self.locale, "🔁 重複播放模式"
                ),
            ...
            embed.add_field(
                name=self.bot.get_text(
                    "display.filters", self.locale, "⚙️ 已啟用效果器"
                ),
                value=", ".join([key.capitalize() for key in self.filters])
                or self.bot.get_text("none", self.locale, "無"),
                inline=True,
            )
            ...
            embed.add_field(
                ...
                value=(
                    self.bot.get_text("display.enable", self.locale, "開啟")
                    if self.shuffle
                    else self.bot.get_text("display.disable", self.locale, "關閉")
                ),
                inline=True,
            )
            ...
            embed.set_footer(
                text=self.bot.get_text(
                    "display.footer",
                    self.locale,
                    "如果你覺得音樂怪怪的，可以試著檢查看看效果器設定或是切換語音頻道地區",
                )
            )
            ...
            embed.title = self.bot.get_text(
                "error.nothing_playing", self.locale, "沒有正在播放的音樂"
            )
    ...
    def __format_time(time_ms: Union[float, int]) -> str:
        ...
        return (
            f"{str(hours).zfill(2)}:" if hours else ""
        ) + f"{str(minutes).zfill(2)}:{str(seconds).zfill(2)}"

    def __generate_progress_bar(
        self, duration: Union[float, int], position: Union[float, int]
    ):
        """
        ...
        return (
            f"{self.bot.get_icon('progress.start_point', 'ST|')}"
