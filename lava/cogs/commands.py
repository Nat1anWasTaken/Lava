...
class Commands(Cog):
    ...
    async def play(
        self, interaction: ApplicationCommandInteraction, query: str, index: int = None
    ):
        await interaction.response.defer()
    ...
    @commands.slash_command(
        name=Localized("guessthesong", key="command.guessthesong.name"),
        description=Localized("猜歌曲", key="command.guessthesong.description"),
        options=[
            Option(name="title", description="遊戲標題", required=True),
            Option(name="genre", description="歌曲類型", required=True),
            Option(name="channel", description="遊戲頻道 (回答答案頻道)", required=False, type=OptionType.channel),
            Option(
                name="rounds", description="遊戲回合數 (預設10回合)", required=False
            ),
            Option(
                name="round_delay",
                description="每一回合開始前時間 (預設5秒)",
                required=False,
            ),
            Option(
                name="round_lenegh",
                description="每回合遊戲長度 (預設30秒)",
                required=False,
            ),
            Option(
                name="victory_score", description="勝利分數 (預設30分)", required=False
            ),
            Option(
                name="match_percentage",
                description="歌曲資訊需符合度 (預設需85%)",
                required=False,
            ),
        ],
    )
    async def guessthesong(
        self,
        interaction: ApplicationCommandInteraction,
        title: str,
        genre: str,
        ...
        round_delay: int = 5,
        round_length: int = 30,
        victory_score: int = 30,
        match_percentage: float = 0.85,
    ):
        if channel is None:
            channel = interaction.channel
        
        game = self.bot.game_manager.new(interaction.guild_id, title, genre, channel, rounds, round_delay, round_length, victory_score, match_percentage)
